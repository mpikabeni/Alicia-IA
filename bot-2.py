...
def register_user(user):
    ...
    con = db()
    con.execute("""
        INSERT INTO users(user_id,first_name,username,messages,last_seen,language_code)
        VALUES(?,?,?,?,?,?)
    ...
    """, (user.id, user.first_name or "", user.username or "", 0, now(), getattr(user, "language_code", None) or "en"))
    con.commit()
    con.close()
...
def update_chat_language(chat_id, user):
    if not chat_id or not user:
        return
    lang = (getattr(user, "language_code", None) or "en").lower().split("-")[0]
    if len(lang) > 12:
        lang = "en"
    con = db()
    con.execute("""
        INSERT INTO chat_languages(chat_id,language_code,updated_at) VALUES(?,?,?)
        ON CONFLICT(chat_id) DO UPDATE SET language_code=excluded.language_code,updated_at=excluded.updated_at
    """, (chat_id, lang, now()))
    con.commit()
    con.close()
...
def get_chat_language(chat_id):
    con = db()
    row = con.execute("SELECT language_code FROM chat_languages WHERE chat_id=?", (chat_id,)).fetchone()
    con.close()
...
def record_message(chat, user, text):
    register_user(user)
    register_chat(chat)
    update_chat_language(chat.id, user)
    con = db()
...
async def ask_ai(chat_id, user_id, user_text):
    ...
    msgs = [{"role": "system", "content": SYSTEM_PROMPT + f"\nRéponds dans la langue Telegram du chat si possible (code: {chat_lang}). Si le message est dans une autre langue, réponds dans la langue utilisée par la personne."}]
...
# Questions come from Open Trivia DB. The source is not limited to anime.
# The poll language follows the Telegram language of the most recently active
# member in each group; unsupported languages are translated automatically.
# ============================================================
QUIZ_INTERVAL_SECONDS = 60 * 60
QUIZ_API = "https://opentdb.com/api.php"
QUIZ_SUPPORTED_LANGUAGES = {"cs", "de", "en", "es", "fr", "hu", "it", "nl", "pt", "tr"}
QUIZ_LANG_ALIASES = {
...
async def translate_quiz_item(item, target_language):
    """Translate a quiz to almost any Telegram language without changing the answer key."""
    if not target_language or target_language in QUIZ_SUPPORTED_LANGUAGES:
        return item
    target_language = QUIZ_LANG_ALIASES.get(target_language, target_language)
    ...
    async with httpx.AsyncClient(timeout=20, follow_redirects=True) as client:
        for text_value in texts:
            try:
                r = await client.get(
                    QUIZ_TRANSLATE_URL,
                    params={"client": "gtx", "sl": "auto", "tl": target_language, "dt": "t", "q": text_value},
                )
                r.raise_for_status()
                data = r.json()
                translated = "".join(part[0] for part in (data[0] or []) if part and part[0]).strip()
                out.append(translated or text_value)
            except Exception:
                out.append(text_value)
    result = dict(item)
    result["question"] = out[0]
    wrong_count = len(item.get("incorrect_answers", []))
    result["incorrect_answers"] = out[1:1 + wrong_count]
...
def quiz_question_key(item):
    raw = "|".join([
        ...
        *[str(x) for x in item.get("incorrect_answers", [])],
    ])
...
def quiz_was_used(chat_id, question_id):
    con = db()
    row = con.execute("SELECT 1 FROM quiz_used WHERE chat_id=? AND question_id=?", (chat_id, question_id)).fetchone()
    con.close()
    return bool(row)
...
def mark_quiz_used(chat_id, question_id):
    con = db()
    con.execute("INSERT OR IGNORE INTO quiz_used(chat_id,question_id,used_at) VALUES(?,?,?)", (chat_id, question_id, now()))
    con.commit()
    con.close()
...
def clear_old_quiz_history(chat_id):
    con = db()
    con.execute("""
        DELETE FROM quiz_used
        WHERE chat_id=? AND used_at < datetime('now','-90 days')
    """, (chat_id,))
    con.commit()
    con.close()
...
async def prepare_quiz(item, chat_id):
    ...
    question = _quiz_clean(item.get("question"))
    correct = _quiz_clean(item.get("correct_answer"))
    ...
    if not question or len(options) != 4:
        return None
    return question, options, correct_index, lang
...
async def send_random_quiz(bot, chat_id, item):
    ...
    if not prepared:
        return False
    question, options, correct_index, lang = prepared
    qid = quiz_question_key(item)
    if quiz_was_used(chat_id, qid):
        return False
    try:
        await bot.send_poll(
            chat_id=chat_id,
            question="🧠 Quiz : " + question[:290],
            options=[x[:100] for x in options],
            type="quiz",
            correct_option_id=correct_index,
            is_anonymous=False,
            explanation="Question aléatoire depuis Open Trivia DB. Chaque groupe reçoit des questions différentes.",
        )
        mark_quiz_used(chat_id, qid)
        return True
    except Exception as e:
        log.warning("Quiz send failed for %s: %s", chat_id, e)
        return False
...
async def quiz_broadcast_once(app):
    ...
    # One API request can provide up to 50 different questions. This avoids
    # hammering OpenTDB (which rate-limits requests).
    pending = [row[0] for row in chats]
    random.shuffle(pending)
    while pending:
        batch_size = min(50, len(pending))
        try:
            items = await fetch_random_quiz(batch_size, "en")
        except Exception as e:
            log.warning("Quiz API unavailable: %s", e)
            return
        if not items:
            return
        used_in_cycle = set()
        for chat_id, item in zip(pending[:batch_size], items):
            qid = quiz_question_key(item)
            if qid in used_in_cycle or quiz_was_used(chat_id, qid):
                # Try another item from this same batch.
                continue
            if await send_random_quiz(app.bot, chat_id, item):
                used_in_cycle.add(qid)
...
async def quiz_scheduler(app):
    # Wait one hour after startup, then repeat every 60 minutes.
...
async def successful_payment(update, context):
    ...
    premium_expiry = (datetime.now(timezone.utc) + timedelta(days=30)).isoformat()
    con.execute("""
        ...
        ON CONFLICT(user_id) DO UPDATE SET
          ...
          stars=excluded.stars, expires_at=excluded.expires_at
    """, (
        u.id, 1, now(), payment.telegram_payment_charge_id, payment.total_amount, premium_expiry
    ))
    con.commit()
    con.close()
    if payment.invoice_payload.startswith("alicia_coding_24h:"):
        ...
        msg=("⭐ Premium activé !\n\nTon Premium est actif pendant 30 jours.\n"
             "🎮 /games\n🎯 /missions\n👤 /profile")
...
async def quiznow(update, context):
    if not admin_ok(update):
        await safe_reply(update.effective_message, "Commande réservée à l'administration.")
        return
    await safe_reply(update.effective_message, "🧠 Je lance les quiz dans tous les groupes enregistrés…")
...
ADMIN_COMMANDS = PUBLIC_COMMANDS + [
    ("quiznow", "Lancer les quiz maintenant"),
