...
FALLBACKS = [
    "Je suis là. Réessaie avec ta question, et je vais te répondre plus clairement.",
    "Hmm, ma connexion avec mon cerveau de secours a eu un petit moment de faiblesse. Réessaie.",
    "J'ai perdu le fil une seconde. Reformule-moi ça et je reprends.",
]
...
def build_ai_prompt(user_text: str, memory_key: str, user_name: str, chat_title: str = "") -> str:
    rows = history_sync(memory_key, 12)
    previous = recent_assistant_sync(memory_key, 5)

    history_lines = []
    for row in rows:
        role = "Utilisateur" if row["role"] == "user" else "Alicia"
        history_lines.append(f"{role}: {row['content'][:700]}")

    avoid_lines = "\n".join(f"- {x[:350]}" for x in previous) if previous else "- Aucun message précédent à éviter."
...
CONTEXTE : {context}
NOM DE LA PERSONNE : {user_name}

HISTORIQUE RÉCENT :
{chr(10).join(history_lines[-12:]) if history_lines else "(début de conversation)"}

FORMULATIONS RÉCENTES D'ALICIA À ÉVITER DE COPIER :
{avoid_lines}

IMPORTANT : réponds à la nouvelle demande avec une formulation différente de tes précédentes réponses. Ne répète pas mot pour mot une phrase déjà utilisée.
...
async def ask_ai(prompt: str) -> tuple[str, str]:
    ...
    logger.error("Tous les fournisseurs IA ont échoué: %s", " | ".join(errors))
...
async def guess_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if state and state.get("game") == "guess":
        ...
        if guess == target:
            ...
            await update.effective_message.reply_text(f"Trouvé. C'était {target}. +5 points.")
        elif guess < target:
            state["attempts"] -= 1
            if state["attempts"] <= 0:
                GAME_STATES.pop(key, None)
                await asyncio.to_thread(add_score_sync, chat.id, user.id, display_name(user), 0, 0, 1)
                await update.effective_message.reply_text(f"Raté. Le nombre était {target}. On retente plus tard.")
            else:
                await update.effective_message.reply_text(f"Plus grand. Il te reste {state['attempts']} tentative(s).")
        else:
            state["attempts"] -= 1
            if state["attempts"] <= 0:
                GAME_STATES.pop(key, None)
                ...
                await update.effective_message.reply_text(f"Plus petit. Il te reste {state['attempts']} tentative(s).")
        return

    GAME_STATES[key] = {"game": "guess", "number": random.randint(1, 20), "attempts": 5}
    await update.effective_message.reply_text("J'ai choisi un nombre entre 1 et 20. Tu as 5 tentatives. Utilise /guess nombre")
...
async def word_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not state or state.get("game") != "word":
        ...
        GAME_STATES[key] = {"game": "word", "word": word, "guessed": set(), "attempts": 6}
        masked = " ".join("_" for _ in word)
        await update.effective_message.reply_text(
            f"Mot mystère\n\n{masked}\n\nEnvoie /word suivi d'une lettre. Exemple : /word A\nTentatives : 6"
        )
        return

    if not context.args:
        await update.effective_message.reply_text("Donne une seule lettre. Exemple : /word A")
        return

    letter = context.args[0].strip().upper()
    if len(letter) != 1 or not letter.isalpha():
        await update.effective_message.reply_text("Choisis une seule lettre.")
        return

    word = state["word"]
    guessed: set[str] = state["guessed"]
    if letter in guessed:
        await update.effective_message.reply_text("Cette lettre a déjà été essayée.")
        return
    guessed.add(letter)
    ...
    if letter not in word:
        ...
        if state["attempts"] <= 0:
            GAME_STATES.pop(key, None)
            await asyncio.to_thread(add_score_sync, chat.id, user.id, display_name(user), 0, 0, 1)
            await update.effective_message.reply_text(f"Dommage. Le mot était {word}.")
            return

    masked = " ".join(ch if ch in guessed else "_" for ch in word)
    if all(ch in guessed for ch in set(word)):
        GAME_STATES.pop(key, None)
        await asyncio.to_thread(add_score_sync, chat.id, user.id, display_name(user), 4, 1, 0)
        await update.effective_message.reply_text(f"Bien joué. Le mot était {word}. +4 points.")
        return

    await update.effective_message.reply_text(
        f"{masked}\n\nTentatives restantes : {state['attempts']}"
    )
...
async def dice_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not user or not chat:
        return
    msg = await update.effective_message.reply_dice()
    if getattr(msg, "dice", None) and msg.dice.value == 6:
        await asyncio.to_thread(add_score_sync, chat.id, user.id, display_name(user), 2, 1, 0)
        await update.effective_message.reply_text("Six. +2 points.")
    else:
        await asyncio.to_thread(add_score_sync, chat.id, user.id, display_name(user), 0, 0, 0)
...
async def coin_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.effective_message.reply_text(f"Résultat : {result}")
...
async def score_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not user or not chat:
        return
    row = await asyncio.to_thread(get_score_sync, chat.id, user.id)
    if not row:
        await update.effective_message.reply_text("Tu n'as pas encore de score. Lance un jeu pour commencer.")
        return
    await update.effective_message.reply_text(
        f"Score de {row['display_name']}\n\n"
        f"Points : {row['points']}\n"
        f"Victoires : {row['wins']}\n"
        f"Défaites : {row['losses']}"
    )
...
async def ranking_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not chat:
        return
    global_scope = chat.type == ChatType.PRIVATE
    rows = await asyncio.to_thread(ranking_sync, chat.id, 10, global_scope)
    if not rows:
        await update.effective_message.reply_text("Le classement est encore vide.")
        return
    title = "Classement global" if global_scope else "Classement du groupe"
    lines = [title, ""]
    for idx, row in enumerate(rows, 1):
        lines.append(f"{idx}. {row['display_name']} — {row['points']} points")
    await update.effective_message.reply_text("\n".join(lines))
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "game:menu":
        await query.answer()
        await query.edit_message_text("Espace Jeux — choisis ton jeu.", reply_markup=game_menu())
        return
    if data in ("game:quizmenu", "game:quiz"):
        await query.answer()
        await send_quiz(update, context, edit_message=query.message if data == "game:quizmenu" else None)
        return
    if data in ("game:quiznext", "game:truthnext"):
        expected_user_id = ACTIVE_QUESTIONS.get((chat.id, query.message.message_id))
        if expected_user_id is not None and expected_user_id != user.id:
            await query.answer("Ce jeu a été lancé par quelqu'un d'autre.", show_alert=True)
            return
        await query.answer()
        if data == "game:quiznext":
            await send_quiz(update, context, edit_message=query.message)
        else:
            await send_truth(update, context, edit_message=query.message)
        return
    if data in ("game:truthmenu", "game:truth"):
        await query.answer()
        await send_truth(update, context, edit_message=query.message if data == "game:truthmenu" else None)
        return
    if data == "game:guessmenu":
        await query.answer()
        await query.edit_message_text("Pour jouer : /guess\n\nPuis envoie /guess suivi d'un nombre entre 1 et 20.")
        return
    if data == "game:wordmenu":
        await query.answer()
        await query.edit_message_text("Pour jouer : /word\n\nPuis envoie /word suivi d'une lettre.")
        return
    if data == "game:dice":
        await query.answer()
        await query.message.reply_text("Lance le dé avec /dice")
        return
    if data == "game:coin":
        await query.answer()
        await query.message.reply_text("Pile ou face : /coin")
        return
    if data == "game:score":
        await query.answer()
        # Affiche le score du joueur qui a cliqué.
        row = await asyncio.to_thread(get_score_sync, chat.id, user.id)
        if not row:
            await query.message.reply_text("Ton score est encore vide.")
        else:
            await query.message.reply_text(
                f"{row['display_name']} : {row['points']} points | {row['wins']} victoire(s) | {row['losses']} défaite(s)"
            )
        return
    if data == "game:ranking":
        await query.answer()
        rows = await asyncio.to_thread(ranking_sync, chat.id, 10, chat.type == ChatType.PRIVATE)
        if not rows:
            await query.message.reply_text("Le classement est encore vide.")
        else:
            title = "Classement global" if chat.type == ChatType.PRIVATE else "Classement du groupe"
            lines = [title, ""]
            for idx, row in enumerate(rows, 1):
                lines.append(f"{idx}. {row['display_name']} — {row['points']} points")
            await query.message.reply_text("\n".join(lines))
        return
    if data in ("help:show", "about:show"):
        await query.answer()
        if data == "help:show":
            await query.edit_message_text(
                "/games — espace jeux\n"
                "/reset — réinitialiser la mémoire\n"
                "/id — identifiant Telegram\n\n"
                "Tu peux aussi simplement me parler dans une conversation privée."
            )
        else:
            await query.edit_message_text(
    ...
    game, qid_s, answer_s = parts[1], parts[2], parts[3]
    if game not in ("quiz", "truth"):
        return
    try:
        qid = int(qid_s)
        answer = int(answer_s)
    ...
    await query.answer()
    dataset = QUIZ_QUESTIONS if game == "quiz" else TRUE_FALSE
    q = next((item for item in dataset if item[0] == qid), None)
    if not q:
        return
    _, prompt_text, options, correct, explanation = q
    is_correct = answer == correct
    if is_correct:
        points = 3 if game == "quiz" else 2
        await asyncio.to_thread(add_score_sync, chat.id, user.id, display_name(user), points, 1, 0)
        result = f"Bonne réponse. +{points} points."
    else:
        await asyncio.to_thread(add_score_sync, chat.id, user.id, display_name(user), 0, 0, 1)
        result = f"Pas cette fois. La bonne réponse était : {options[correct]}."

    result_text = f"{result}\n\n{explanation}"
    next_data = "game:quizmenu" if game == "quiz" else "game:truthmenu"
    markup = InlineKeyboardMarkup([[InlineKeyboardButton("Question suivante", callback_data=("game:quiznext" if game == "quiz" else "game:truthnext"))], [InlineKeyboardButton("Jeux", callback_data="game:menu")]])
    await query.edit_message_text(result_text, reply_markup=markup)
    ACTIVE_QUESTIONS.pop((chat.id, query.message.message_id), None)
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    memory_key = await save_context(update, user)

    if chat.type in (ChatType.GROUP, ChatType.SUPERGROUP):
        bot_id = context.bot.id
        if not is_called_in_group(update, bot_id):
            return
        text = clean_group_text(message.text)
        if not text:
            text = "Tu voulais m'appeler ?"
    else:
        text = message.text.strip()

    await asyncio.to_thread(save_message_sync, memory_key, chat.id, user.id, "user", text)
    ...
    await asyncio.to_thread(save_message_sync, memory_key, chat.id, user.id, "assistant", answer)

    logger.info("ALICIA réponse via %s | chat=%s user=%s", provider, chat.id, user.id)
...
async def sticker_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if chat.type in (ChatType.GROUP, ChatType.SUPERGROUP) and not reply_is_from_bot(message, context.bot.id):
        return

    memory_key = await save_context(update, user)
    sticker_emoji = message.sticker.emoji or ""
    content = f"[Autocollant reçu{f' {sticker_emoji}' if sticker_emoji else ''}]"
    await asyncio.to_thread(save_message_sync, memory_key, chat.id, user.id, "user", content)

    prompt = build_ai_prompt(
        user_text="La personne vient de m'envoyer un autocollant Telegram. Réagis naturellement à l'autocollant sans inventer précisément ce qu'il représente. L'emoji associé est : " + sticker_emoji,
        memory_key=memory_key,
        user_name=display_name(user),
        chat_title=chat.title or "",
    )
    answer, _ = await ask_ai(prompt)
    await asyncio.to_thread(save_message_sync, memory_key, chat.id, user.id, "assistant", answer)
    await message.reply_text(answer)
...
async def chat_member_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not cm or not cm.new_chat_member or not cm.from_user or not cm.chat:
        return
    ...
    joined = new_status in (ChatMember.MEMBER, ChatMember.ADMINISTRATOR, ChatMember.OWNER) and old_status in (
        ChatMember.LEFT,
        ChatMember.KICKED,
    )
    left = new_status in (ChatMember.LEFT, ChatMember.KICKED) and old_status in (
        ChatMember.MEMBER,
    ...
    if joined:
        variations = [
            f"Bienvenue {display_name(target)}. Installe-toi, on va bien s'entendre.",
            f"Bienvenue parmi nous, {display_name(target)}. J'espère que tu vas rester un peu.",
            f"Hey {display_name(target)}, bienvenue. Fais comme chez toi.",
        ]
        await context.bot.send_message(cm.chat.id, random.choice(variations))
    elif left:
        variations = [
            f"{display_name(target)} nous quitte. Bon chemin à toi.",
            f"Dommage de te voir partir, {display_name(target)}. À une prochaine.",
            f"{display_name(target)} est parti. Le groupe va sentir ton absence.",
        ]
        await context.bot.send_message(cm.chat.id, random.choice(variations))
...
async def warn_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not await require_group_admin(update, context):
        return
    target = await target_user_from_reply(update)
    if not target:
        await update.effective_message.reply_text("Réponds au message de la personne à avertir.")
        return
    if target.is_bot:
        await update.effective_message.reply_text("Je ne vais pas avertir un autre bot.")
        return
    count = await asyncio.to_thread(add_warning_sync, update.effective_chat.id, target.id)
    await update.effective_message.reply_text(f"Avertissement pour {display_name(target)} : {count}.")
...
async def warns_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    target = await target_user_from_reply(update)
    if not target:
        await update.effective_message.reply_text("Réponds au message de la personne.")
        return
    count = await asyncio.to_thread(warnings_sync, update.effective_chat.id, target.id)
    await update.effective_message.reply_text(f"{display_name(target)} a {count} avertissement(s).")
...
async def unwarn_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not target:
        await update.effective_message.reply_text("Réponds au message de la personne.")
        return
    await asyncio.to_thread(clear_warnings_sync, update.effective_chat.id, target.id)
    await update.effective_message.reply_text(f"Les avertissements de {display_name(target)} ont été réinitialisés.")
...
async def ban_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    target = await target_user_from_reply(update)
    if not target:
        await update.effective_message.reply_text("Réponds au message de la personne à bannir.")
        return
    try:
        await context.bot.ban_chat_member(update.effective_chat.id, target.id)
        await update.effective_message.reply_text(f"{display_name(target)} a été exclu du groupe.")
    except Exception as exc:
        logger.warning("ban error: %r", exc)
        await update.effective_message.reply_text("Je n'ai pas pu effectuer le bannissement. Vérifie mes droits d'administration.")
...
async def unban_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not await require_group_admin(update, context):
        return
    if not context.args:
        await update.effective_message.reply_text("Utilise /unban ID_UTILISATEUR")
        return
    try:
        target_id = int(context.args[0])
        await context.bot.unban_chat_member(update.effective_chat.id, target_id, only_if_banned=True)
        await update.effective_message.reply_text("Utilisateur débanni.")
    except Exception as exc:
        logger.warning("unban error: %r", exc)
        await update.effective_message.reply_text("Je n'ai pas pu débannir cet utilisateur.")
...
async def mute_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    target = await target_user_from_reply(update)
    if not target:
        await update.effective_message.reply_text("Réponds au message de la personne à mettre en sourdine. Exemple : /mute 10m")
        return
    duration = parse_duration(context.args[0]) if context.args else 3600
    ...
    try:
        ...
        await update.effective_message.reply_text(f"{display_name(target)} est muet pendant {duration // 60} minute(s).")
    except Exception as exc:
        logger.warning("mute error: %r", exc)
        await update.effective_message.reply_text("Je n'ai pas pu appliquer le mute. Vérifie mes droits.")
...
async def unmute_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    target = await target_user_from_reply(update)
    if not target:
        await update.effective_message.reply_text("Réponds au message de la personne à démuter.")
        return
    ...
    try:
        ...
        await update.effective_message.reply_text(f"{display_name(target)} peut à nouveau parler.")
    except Exception as exc:
        logger.warning("unmute error: %r", exc)
        await update.effective_message.reply_text("Je n'ai pas pu retirer le mute.")
...
async def clear_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not message.reply_to_message:
        await message.reply_text("Réponds au message que tu veux supprimer.")
        return
    try:
        await message.reply_to_message.delete()
        await message.delete()
    except Exception as exc:
        logger.warning("clear error: %r", exc)
        await message.reply_text("Je n'ai pas pu supprimer ce message.")
...
async def resetuser_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    target = await target_user_from_reply(update)
    if not target:
        await update.effective_message.reply_text("Réponds au message de la personne dont tu veux effacer la mémoire.")
        return
    count = await asyncio.to_thread(reset_user_sync, update.effective_chat.id, target.id)
    await update.effective_message.reply_text(f"Mémoire de {display_name(target)} réinitialisée ({count} message(s) supprimé(s)).")


async def resetgroup_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not await require_group_admin(update, context):
        return
    count = await asyncio.to_thread(reset_group_sync, update.effective_chat.id)
    await update.effective_message.reply_text(f"Mémoire du groupe réinitialisée ({count} message(s)).")
...
async def memory_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not user or not chat:
        return
    key = make_memory_key(chat.id, user.id, chat.type)
    count = await asyncio.to_thread(count_memory_sync, key)
    await update.effective_message.reply_text(f"Mémoire associée à cette conversation : {count} message(s).")
...
async def admin_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.effective_message.reply_text(
        ...
        "/clear, /resetuser, /resetgroup, /resetall\n/restart"
    )
...
async def stats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not await require_admin(update):
        return
    users = await asyncio.to_thread(count_users_sync)
    groups = await asyncio.to_thread(count_groups_sync)
    await update.effective_message.reply_text(
        f"Alicia v{VERSION}\nUtilisateurs connus : {users}\nGroupes connus : {groups}\n"
        f"Groq : {'OK' if groq_client else 'absent'}\nGemini : {'OK' if gemini_client else 'absent'}"
    )
...
async def users_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not await require_admin(update):
        return
    users = await asyncio.to_thread(get_private_chat_ids_sync)
    await update.effective_message.reply_text(f"Abonnés privés connus : {len(users)}")


async def groups_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not await require_admin(update):
        return
    groups = await asyncio.to_thread(get_group_chat_ids_sync)
    await update.effective_message.reply_text(f"Groupes connus : {len(groups)}")
...
async def broadcast_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    chat_ids = await asyncio.to_thread(get_private_chat_ids_sync)
    sent = 0
    failed = 0
    for chat_id in chat_ids:
        try:
            ...
            failed += 1
    await update.effective_message.reply_text(f"Broadcast terminé. Envoyés : {sent}. Échecs : {failed}.")


async def broadcastgroups_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not await require_admin(update):
        return
    if not context.args:
        ...
        return
    text = " ".join(context.args).strip()
    chat_ids = await asyncio.to_thread(get_group_chat_ids_sync)
    sent = 0
    failed = 0
    for chat_id in chat_ids:
        try:
            ...
            failed += 1
    await update.effective_message.reply_text(f"Broadcast groupes terminé. Envoyés : {sent}. Échecs : {failed}.")


async def resetall_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not await require_admin(update):
        return
    await asyncio.to_thread(reset_all_sync)
    GAME_STATES.clear()
    ACTIVE_QUESTIONS.clear()
    await update.effective_message.reply_text("Toutes les mémoires, scores, avertissements et questions utilisées ont été réinitialisés.")


async def restart_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not await require_admin(update):
        return
    await update.effective_message.reply_text("Je redémarre le service. À tout de suite.")
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        if isinstance(update, Update) and update.effective_message:
            await update.effective_message.reply_text("J'ai rencontré un petit problème technique. Réessaie.")
    except Exception:
        pass
...
async def post_init(application: Application) -> None:
    ...
    logger.info("ALICIA v%s connectée sous %s", VERSION, me.username)
    commands = [
        ("start", "Démarrer Alicia"),
        ("help", "Aide"),
        ...
        ("ranking", "Voir le classement"),
        ("reset", "Réinitialiser la mémoire"),
        ("id", "Voir son identifiant"),
    ]
    try:
        await application.bot.set_my_commands(commands)
    except Exception as exc:
        logger.warning("Impossible de définir les commandes Telegram: %r", exc)


def get_render_base_url() -> str:
    url = RENDER_EXTERNAL_URL.rstrip("/")
    if not url:
        # Sur Render, cette variable peut être absente si l'utilisateur a oublié de la définir.
        # On ne tente pas d'inventer un domaine.
        return ""
    return url
...
def build_application() -> Application:
    ...
    app.add_handler(CommandHandler("resetgroup", resetgroup_cmd))
    ...
    app.add_handler(CommandHandler("broadcastgroups", broadcastgroups_cmd))
...
def main() -> None:
    ...
    base = get_render_base_url()

    if base:
        webhook_url = f"{base}/telegram"
        logger.info("Mode Render Webhook")
        logger.info("URL Render : %s", base)
