...
# Groq est utilisé comme fournisseur IA principal.
...
# Protection contre les appels répétés pendant une limitation du fournisseur IA.
OPENAI_COOLDOWN_SECONDS = int(os.getenv("OPENAI_COOLDOWN_SECONDS", "1800"))
...
async def message_handler(
    ...
    # Si le fournisseur IA vient de nous imposer une limite, ne gaspille pas
    # d'autres requêtes pendant la période de refroidissement.
    if time.time() < openai_cooldown_until:
        await update.message.reply_text(
            "Aïe 😭 j'ai beaucoup trop parlé aujourd'hui... "
            "Laisse-moi reprendre mon souffle un petit moment 😂"
        )
        return

    try:

        # Chat Completions est utilisé ici pour une compatibilité simple
        # et robuste avec les modèles Groq.
        messages = [
        ...
        if (
            ...
            openai_cooldown_until = time.time() + OPENAI_COOLDOWN_SECONDS

            await update.message.reply_text(
                "Oups 😭 j'ai atteint ma limite de messages pour le moment. "
                "Je fais une petite pause et je reviens dès que possible 💜"
            )
        else:
            await update.message.reply_text(
...
async def telegram_error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    # Rend les erreurs visibles dans les logs Render au lieu du simple
    # message "No error handlers are registered".
    print("===== ERREUR TELEGRAM/ALICIA =====")
    print(f"Update : {update!r}")
    print(f"Exception : {context.error!r}")
    print("==================================")
...
def main():
    ...
    if not GROQ_API_KEY:
        print("❌ GROQ_API_KEY manque dans les variables d'environnement")
        return
