...
def is_alicia_mentioned(update: Update) -> bool:
    ...
    # Mention Telegram via entité text_mention ou mention.
    if message.entities:
        for entity in message.entities:
            if entity.type == "mention":
                ...
                me = update.get_bot().id
                if entity.user.id == me:
                    return True

    # Réponse directe au message d'ALICIA.
...
def clean_group_message(text: str) -> str:
    """Retire la mention d'ALICIA pour donner à l'IA un message naturel."""
