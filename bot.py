...
def register_user(user):
    ...
    try:
        ...
        """, (user.id, user.first_name or "", user.username or "", 0, now(), lang))
    except sqlite3.OperationalError:
        # Compatibility with an older database before the migration is committed.
        con.execute("""
...
def update_chat_language(chat_id, user):
    if not chat_id or not user:
        return
    lang = (getattr(user, "language_code", None) or "en").lower().replace("_", "-")
    lang = lang.split("-")[0][:12]
    con = db()
    con.execute("""
...
async def ask_ai(chat_id, user_id, user_text):
    ...
    language_instruction = f"\nLangue Telegram détectée pour ce chat : {lang}. Réponds dans cette langue sauf si l'utilisateur écrit clairement dans une autre langue."
...
async def translate_quiz_item(item, target_language):
    ...
    if target_language in QUIZ_SUPPORTED_LANGUAGES:
        return item
...
async def send_random_quiz(bot, chat_id, item):
    ...
    try:
        await bot.send_poll(chat_id=chat_id, question="🧠 Quiz : " + question[:290], options=[x[:100] for x in options], type="quiz", correct_option_id=correct_index, is_anonymous=False, explanation="Quiz aléatoire du jour. Question différente autant que possible pour chaque groupe.")
        mark_quiz_used(chat_id, qid)
        return True
    except Exception as e:
        log.warning("Quiz send failed %s: %s", chat_id, e)
        return False

async def quiz_broadcast_once(app):
    con = db()
    chats = [r[0] for r in con.execute("SELECT chat_id FROM chats WHERE chat_type IN ('group','supergroup')").fetchall()]
    con.close()
    if not chats:
        return
    random.shuffle(chats)
    # Up to 50 questions per API call, so large deployments do not hammer the source.
    for start in range(0, len(chats), 50):
        batch = chats[start:start+50]
        try:
            items = await fetch_random_quiz(len(batch))
        except Exception as e:
        ...
        for chat_id in batch:
            chosen = next((x for x in items if quiz_question_key(x) not in used_cycle and not quiz_was_used(chat_id, quiz_question_key(x))), None)
            if chosen and await send_random_quiz(app.bot, chat_id, chosen):
                used_cycle.add(quiz_question_key(chosen))
        if start + 50 < len(chats):
...
PUBLIC_COMMANDS = [
    ...
    ("missions","Missions Premium"),("premium","Premium 30 jours"),("coding","Codage 50 ⭐ / 24h"),("projet","Mes projets de codage"),
...
ADMIN_COMMANDS = PUBLIC_COMMANDS + [
    ("quiznow", "Lancer un quiz aléatoire"),
