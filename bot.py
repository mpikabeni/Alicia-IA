...
Réponds court quand c’est simple, varie tes formulations et évite les réponses robotiques ou répétitives.
...
async def ask_ai(chat_id, user_id, user_text):
    ...
    for name, fn in providers:
        if not provider_ready(name):
            continue
        try:
            result = await fn(); _PROVIDER_USED[name] = _PROVIDER_USED.get(name,0)+1; globals()["_LAST_PROVIDER"] = name; return result
        except Exception as e:
            errors.append(f"{name}: {type(e).__name__}: {e}")
            text = str(e).lower()
...
async def quiz_broadcast_once(app):
    con=db(); chats=[r[0] for r in con.execute("SELECT chat_id FROM chats WHERE chat_type IN ('group','supergroup') ORDER BY chat_id").fetchall()]; con.close()
    if not chats: return
...
async def quiz_scheduler(app):
    ...
    while True:
        try: await quiz_broadcast_once(app)
        except asyncio.CancelledError: raise
        except Exception: log.exception("Quiz scheduler error")
        await asyncio.sleep(60*60)


    # First quiz one hour after startup, then every 60 minutes.
...
async def rss_cmd(update,context):
    chat=update.effective_chat; uid=(update.effective_user.id if update.effective_user else ADMIN_USER_ID)
    if update.effective_user: register_user(update.effective_user)
    register_chat(chat)
    args=context.args or []
    if not args:
        await safe_reply(update.effective_message,"📡 RSS\n/rss add URL — ajouter un flux à ce chat\n/rss list — voir les flux actifs\n/rss remove ID — désactiver un flux dans ce chat\n\nLes flux peuvent venir de sites web, Google News, Facebook/Twitter via une URL RSS compatible, etc.")
        return
    action=args[0].lower()
    if action=='add' and len(args)>=2:
        url=args[1].strip(); parsed=urlparse(url)
        if parsed.scheme not in ('http','https'):
            await safe_reply(update.effective_message,"URL invalide."); return
        try:
            items=await rss_fetch(url)
            if not items: raise RuntimeError("Aucun article RSS détecté")
        except Exception as e:
            await safe_reply(update.effective_message,f"❌ Flux inaccessible : {e}"); return
        con=db(); con.execute("INSERT OR IGNORE INTO rss_feeds(owner_user_id,url,title,active,created_at) VALUES(?,?,?,?,?)",(uid,url,parsed.netloc,1,now())); feed=con.execute("SELECT id FROM rss_feeds WHERE url=?",(url,)).fetchone()[0]
        con.execute("INSERT OR REPLACE INTO rss_routes(feed_id,destination_chat_id,owner_user_id,active,created_at) VALUES(?,?,?,?,?)",(feed,chat.id,uid,1,now())); con.commit(); con.close()
        await safe_reply(update.effective_message,f"✅ Flux activé dans ce chat. {len(items)} articles détectés.")
        return
    con=db()
    if action=='list':
        rows=con.execute("SELECT f.id,f.title,f.url FROM rss_feeds f JOIN rss_routes r ON r.feed_id=f.id WHERE r.destination_chat_id=? AND r.active=1 AND f.active=1 ORDER BY f.id",(chat.id,)).fetchall(); con.close()
        if not rows: await safe_reply(update.effective_message,"Aucun flux actif ici."); return
        await safe_reply(update.effective_message,"📡 FLUX ACTIFS\n"+"\n".join(f"#{r[0]} — {r[1]}\n{r[2]}" for r in rows)); return
    if action=='remove' and len(args)>=2:
        try: fid=int(args[1])
        except: await safe_reply(update.effective_message,"ID invalide."); return
        con.execute("UPDATE rss_routes SET active=0 WHERE feed_id=? AND destination_chat_id=?",(fid,chat.id)); con.commit(); con.close(); await safe_reply(update.effective_message,"✅ Flux désactivé dans ce chat."); return
    con.close(); await safe_reply(update.effective_message,"Commande RSS inconnue. Utilise /rss add URL ou /rss list.")

async def rss_broadcast_once(app):
    con=db(); feeds=con.execute("SELECT id,url FROM rss_feeds WHERE active=1").fetchall(); con.close()
    for fid,url in feeds:
        try: items=await rss_fetch(url)
        except Exception as e: log.warning("RSS %s: %s",url,e); continue
        for key,title,link,pub in reversed(items):
            con=db(); exists=con.execute("SELECT 1 FROM rss_items WHERE feed_id=? AND item_key=?",(fid,key)).fetchone()
            if exists: con.close(); continue
            con.execute("INSERT OR IGNORE INTO rss_items(feed_id,item_key,title,link,published_at,created_at) VALUES(?,?,?,?,?,?)",(fid,key,title,link,pub,now()))
            routes=con.execute("SELECT destination_chat_id FROM rss_routes WHERE feed_id=? AND active=1",(fid,)).fetchall(); con.commit(); con.close()
...
async def rss_scheduler(app):
    ...
    while True:
        try: await rss_broadcast_once(app)
        except asyncio.CancelledError: raise
        except Exception: log.exception("RSS scheduler error")
...
PUBLIC_COMMANDS = [
    ...
    ("groupstats","Stats du groupe"),("dashboard","Tableau de bord"),("top","Top du groupe"),("alltime","Meilleurs joueurs"),("badges","Mes badges"),("friends","Entourage d'Alicia"),("tournament","Tournoi"),("remind","Rappel"),("weather","Météo"),("time","Heure"),("convert","Conversion"),("search","Recherche web"),("translate","Traduction"),("voice","Vocal Alicia"),("rss","Flux RSS")
]
