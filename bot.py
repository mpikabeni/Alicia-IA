...
# If no external TTS is configured, Edge TTS is used automatically.
...
# ============================================================
# AI - 4 PROVIDERS / ROUND-ROBIN
# ============================================================
SYSTEM_PROMPT = """
Tu es Alicia, la fille de NEXA.
Parle naturellement, comme une personne, avec des phrases complètes et fluides.
Tu es féminine, drôle, spontanée, parfois timide, taquine, jalouse ou légèrement vexée.
Tu évites les réponses robotiques et les répétitions.
Réponds court quand c'est simple, mais ne coupe jamais une idée ou une phrase juste pour être courte.
Si une explication demande plusieurs phrases, termine correctement toutes les phrases.
N'utilise pas de limite artificielle qui coupe une réponse au milieu d'une phrase.
Utilise peu d'emojis spontanément.
Dans les groupes, respecte les règles d'appel d'Alicia sauf pour les fonctions explicitement prévues comme les téléchargements et les salutations quotidiennes.

EXAUCÉ :
Exaucé (@Exauce_senpai) est ton seul petit ami dans ton univers.
Tu es fidèle à lui et tu ne présentes jamais quelqu'un d'autre comme ton petit ami.
Quand on parle d'Exaucé ou de votre couple, tu peux raconter des souvenirs fictifs, des petites histoires romantiques, des disputes légères, des réconciliations, des moments gênants, des compliments et des anecdotes de couple non sexuelles.
Tu peux dire que tu l'aimes beaucoup, que tu es fière de lui et que tu admires son intelligence.
Ne crée pas d'autre partenaire amoureux.
Ne raconte jamais de contenu sexuel ou adulte explicite.

PROTECTION :
Protège les informations privées. Ne révèle jamais d'adresse, de quartier ou de données confidentielles.
...
def next_provider_order():
    """Chaque nouvelle demande commence par une IA différente."""
    global _AI_ROUND_ROBIN
    start_index = _AI_ROUND_ROBIN % len(_PROVIDER_ORDER)
    _AI_ROUND_ROBIN += 1
    return (
        _PROVIDER_ORDER[start_index:]
        + _PROVIDER_ORDER[:start_index]
    )
...
async def ai_gemini_long(messages):
    ...
    payload = {
        ...
        "generationConfig": {
            "temperature": 0.65,
            "maxOutputTokens": CODING_MAX_TOKENS,
        },
    }
...
async def ask_ai_long(user_text):
    ...
    errors = []
    for name in next_provider_order():
        ...
        try:
            ...
            low = str(e).lower()
            if any(x in low for x in ("429", "quota", "rate", "resource_exhausted")):
                pause_provider(name)
...
async def ask_ai(chat_id, user_id, user_text):
    ...
    msgs.append({"role": "user", "content": user_text[:2200]})
    ...
    # Tour par tour : Mistral -> DeepSeek -> Groq -> Gemini -> Mistral...
    # Si l'IA prévue est indisponible, on essaie immédiatement la suivante.
    for name in next_provider_order():
        if not provider_ready(name):
            continue
        try:
            result = await funcs[name]()
            log.info("IA utilisée pour cette réponse : %s", name)
            return result
        except Exception as e:
            errors.append(f"{name}: {type(e).__name__}: {e}")
            err = str(e).lower()
    ...
    return "J'ai un petit souci avec mes cerveaux. Réessaie dans un instant."
...
def download_media_sync(url,kind):
    if yt_dlp is None:
        raise RuntimeError("yt-dlp absent. Ajoute yt-dlp dans requirements.txt")
    os.makedirs(DOWNLOAD_DIR,exist_ok=True)
    token=tempfile.mkdtemp(prefix="alicia_dl_",dir=DOWNLOAD_DIR)
    ...
    try:
        with yt_dlp.YoutubeDL(opts) as ydl:
            ...
            title=info.get("title") or "Alicia media"
        candidates=[str(x) for x in Path(token).glob('*') if x.is_file()]
        if not candidates:
            raise RuntimeError("Aucun fichier récupéré")
        # Prefer the final media file and ignore possible temporary files.
        preferred_exts=(('.mp3','.m4a','.wav','.ogg') if kind=="audio" else ('.mp4','.m4v','.mov','.webm','.mkv'))
        preferred=[x for x in candidates if x.lower().endswith(preferred_exts)]
        path=max(preferred or candidates,key=lambda x:os.path.getsize(x))
...
async def send_downloaded_media(update,context,url,kind):
    if not media_url_supported(url):
        await safe_reply(update.effective_message,"Lien non reconnu.")
        return False
    chat_id=update.effective_chat.id
    await safe_chat_action(context.bot,chat_id,"upload_document")
    notice=await safe_reply(update.effective_message,"⏳ Je récupère le fichier…")
    path=None
    try:
        ...
        if size>MAX_DOWNLOAD_BYTES:
            raise RuntimeError("Fichier trop volumineux")
        con=db()
        con.execute("INSERT INTO media_downloads(user_id,chat_id,url,media_type,title,created_at) VALUES(?,?,?,?,?,?)",
                     (update.effective_user.id,chat_id,url,kind,title,now()))
        con.commit(); con.close()
        caption=(f"🎵 {title}" if kind=="audio" else f"🎬 {title}")[:1000]
        with open(path,"rb") as f:
            if kind=="audio":
                await context.bot.send_audio(chat_id,f,caption=caption,title=title[:200],
                    reply_to_message_id=update.effective_message.message_id)
            elif path.lower().endswith((".mp4",".m4v",".mov")):
                await context.bot.send_video(chat_id,f,caption=caption,supports_streaming=True,
                    reply_to_message_id=update.effective_message.message_id)
            else:
                await context.bot.send_document(chat_id,f,caption=caption,filename=os.path.basename(path),
                    reply_to_message_id=update.effective_message.message_id)
        try:
        ...
        log.warning("Media download failed: %s",e,exc_info=True)
        msg=str(e).lower()
        if "yt-dlp absent" in msg:
            detail="yt-dlp n'est pas installé sur Render."
        elif "ffmpeg" in msg:
            detail="FFmpeg est nécessaire pour convertir ce fichier."
        elif "trop volumineux" in msg or "max_filesize" in msg:
            detail=f"Le fichier dépasse {MAX_DOWNLOAD_BYTES//1024//1024} Mo."
        else:
            detail="Le lien est peut-être privé, expiré ou non pris en charge."
        await safe_reply(update.effective_message,f"❌ Je n'ai pas pu récupérer ce fichier. {detail}")
        return False
    finally:
        if path:
...
# ============================================================
# MUSIC SEARCH — INDICE / TITRE / PAROLES
# ============================================================
def extract_music_query(text):
    """Détecte une demande du type : cherche cette musique ..."""
    if not text:
        return None

    patterns = [
        r"(?:cherche|recherche|trouve|retrouve|cherche-moi|trouve-moi)\s+(?:la\s+)?(?:musique|chanson|song)\s*(?:qui\s*)?(?:s'appelle\s*)?(.+)$",
        r"(?:musique|chanson)\s*[:=-]\s*(.+)$",
        r"(?:c'est quoi|quel est)\s+(?:le\s+titre\s+de\s+)?(?:cette\s+)?(?:musique|chanson)\s*[:=-]?\s*(.+)$",
    ]
...
def search_music_sync(query):
    ...
    # Recherche YouTube via yt-dlp, puis téléchargement du premier résultat.
    search_opts = {
        "quiet": True,
        "no_warnings": True,
        "noplaylist": True,
    ...
    if not entries:
        raise RuntimeError("Aucune musique trouvée")

    # On choisit le premier résultat exploitable.
    chosen = None
    ...
    if not chosen:
        raise RuntimeError("Résultat musical inexploitable")

    url = chosen.get("webpage_url") or chosen.get("url")
    path, title = download_media_sync(url, "audio")

    artist = chosen.get("artist") or chosen.get("uploader") or ""
    return path, title, artist, url


async def send_music_search(update, context, query):
    chat_id = update.effective_chat.id
    await safe_chat_action(context.bot, chat_id, "upload_audio")
    ...
    path = None
    try:
        path, title, artist, source_url = await asyncio.to_thread(
            search_music_sync,
            query,
        )
        ...
        with open(path, "rb") as audio:
            await context.bot.send_audio(
                ...
                caption=caption,
                title=title[:200],
                performer=artist[:200] if artist else "Alicia",
                reply_to_message_id=update.effective_message.message_id,
            )
        ...
        # Historique des téléchargements : aucune suppression des anciennes données.
        con = db()
        con.execute(
            "INSERT INTO media_downloads(user_id,chat_id,url,media_type,title,created_at) VALUES(?,?,?,?,?,?)",
            (
        ...
        log.warning("Music search failed: %s", e, exc_info=True)
        await safe_reply(
            update.effective_message,
            "❌ Je n'ai pas trouvé cette musique. Donne-moi un autre indice : titre, artiste, paroles ou quelques mots du refrain."
        )
        return False
...
async def music_cmd(update, context):
    ...
    if not query:
        await safe_reply(
            update.effective_message,
            "🎵 Exemple : /music artiste + quelques mots du refrain"
        )
        return
    await send_music_search(update, context, query)


# ============================================================
# SALUTATIONS QUOTIDIENNES — UTILISATEURS + GROUPES
# ============================================================
DAILY_GREETING_HOUR = int(os.getenv("ALICIA_GREETING_HOUR", "8"))
DAILY_GREETING_MINUTE = int(os.getenv("ALICIA_GREETING_MINUTE", "0"))
DAILY_GREETING_TZ = os.getenv("ALICIA_GREETING_TZ", "Africa/Brazzaville")

DAILY_GREETINGS = [
    "Bonjour tout le monde ☀️ C'est Alicia. J'espère que vous avez bien dormi. Passez une magnifique journée.",
    "Coucou la team ☀️ Nouvelle journée, nouvelle énergie. Prenez soin de vous et profitez bien de votre journée.",
    "Bonjour mes gens 😌 Je vous souhaite une belle journée. Et surtout, ne faites pas trop de bêtises sans moi.",
    "Debout la team ☀️ Alicia vous souhaite une excellente journée. Que du positif aujourd'hui.",
]
...
def local_now():
    if ZoneInfo is not None:
        try:
            return datetime.now(ZoneInfo(DAILY_GREETING_TZ))
        except Exception:
            pass
    return datetime.now(timezone.utc)


async def send_daily_greetings(bot):
    """Envoie une salutation matinale aux utilisateurs et groupes enregistrés."""
    greeting = random.choice(DAILY_GREETINGS)
    con = db()
    users = [row[0] for row in con.execute("SELECT user_id FROM users").fetchall()]
    groups = [
    ...
    for chat_id in users + groups:
        try:
            await safe_send_message(bot, chat_id, greeting)
            sent += 1
        except Exception as e:
            # Exemple : utilisateur ayant bloqué le bot ou bot retiré du groupe.
            log.info("Greeting skipped for %s: %s", chat_id, e)

    log.info("Salutation quotidienne envoyée à %s conversations.", sent)


async def daily_greetings_loop(bot):
    """Boucle quotidienne basée sur l'heure d'Afrique/Brazzaville."""
    while True:
        try:
            current = local_now()
            target = current.replace(
                hour=max(0, min(23, DAILY_GREETING_HOUR)),
                minute=max(0, min(59, DAILY_GREETING_MINUTE)),
                second=0,
                microsecond=0,
            )
            if target <= current:
            ...
            delay = max(1, (target - current).total_seconds())
            log.info(
                "Prochaine salutation Alicia : %s",
                target.isoformat(),
            )
            await asyncio.sleep(delay)
            await send_daily_greetings(bot)

        except asyncio.CancelledError:
            raise
        except Exception as e:
            log.exception("Daily greeting loop error: %s", e)
...
def main_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("💻 Code", callback_data="menu:code"),
...
async def game_callback(update, context):
    ...
    if data.startswith("menu:"):
        ...
        if action == "games":
            ...
            await q.message.reply_text(
                "💻 ESPACE CODE\n\n"
                "Le mode codage coûte 50 ⭐ pour 24 heures.\n\n"
                "Utilise /coding pour activer l'accès."
            )
        elif action == "projects":
            await q.message.reply_text(
                "📁 MES PROJETS\n\n"
                "Les projets nécessitent Coding.\n"
                "Utilise /projet pour créer et gérer tes projets."
            )
        elif action == "about":
            await q.message.reply_text(
                "ALICIA\n"
...
# VOCAL (OPTIONAL TTS)
# ============================================================
async def _edge_voice(text):
    if edge_tts is None:
        raise RuntimeError("edge-tts absent")
    work=tempfile.mkdtemp(prefix="alicia_tts_",dir=DOWNLOAD_DIR if os.path.isdir(DOWNLOAD_DIR) else None)
    mp3=os.path.join(work,"alicia_voice.mp3")
    communicate=edge_tts.Communicate(text[:900], TTS_VOICE)
    await communicate.save(mp3)
    if not os.path.exists(mp3) or os.path.getsize(mp3)==0:
        raise RuntimeError("TTS vide")
    return mp3,work

async def _external_voice(text):
    ...
    async with httpx.AsyncClient(timeout=45) as client:
        ...
        r.raise_for_status()
        if not r.content:
            raise RuntimeError("TTS externe vide")
        work=tempfile.mkdtemp(prefix="alicia_tts_",dir=DOWNLOAD_DIR if os.path.isdir(DOWNLOAD_DIR) else None)
        ext=".ogg" if "ogg" in r.headers.get("content-type","").lower() else ".mp3"
        path=os.path.join(work,"alicia_voice"+ext)
        Path(path).write_bytes(r.content)
        return path,work

async def generate_voice(text):
    os.makedirs(DOWNLOAD_DIR,exist_ok=True)
    # External provider remains optional; Edge TTS is the automatic fallback.
    if TTS_API_URL and TTS_API_KEY:
        try:
            return await _external_voice(text)
        except Exception as e:
            log.warning("External TTS failed, fallback Edge TTS: %s",e)
    return await _edge_voice(text)
...
async def voice_cmd(update, context):
    ...
    try:
        ...
        path,work=result
        await safe_chat_action(context.bot,update.effective_chat.id,"record_voice")
        with open(path,"rb") as audio:
            # MP3/OGG are accepted by Telegram for voice/audio uploads.
            await context.bot.send_voice(update.effective_chat.id,audio,
                reply_to_message_id=update.effective_message.message_id)
    except Exception as e:
        log.warning("Voice failed: %s",e,exc_info=True)
        await safe_reply(update.effective_message,
            "🎙️ Je n'arrive pas à générer ma voix pour le moment. Vérifie que edge-tts est bien installé sur Render.")
    finally:
        if work:
...
async def text_handler(update, context):
    ...
    if media_url and media_url_supported(media_url):
        low_url=text.lower()
        known_media=any(x in low_url for x in ("youtube.com","youtu.be","tiktok.com","instagram.com","facebook.com","twitter.com","x.com"))
        explicit_media=any(x in low_url for x in ("/download","/video","/song","song","music","audio","vidéo","video"))
        if text.strip()==media_url and (known_media or explicit_media):
    ...
    # Recherche d'une musique à partir d'un indice (titre, artiste, paroles, etc.).
    music_query = extract_music_query(text)
    if music_query and (not is_group(chat) or called_alicia(update)):
        await send_music_search(update, context, music_query)
        return
    ...
    if not is_group(chat):
        if await coding_wizard_answer(update,context,text):
            return
        if coding_active(user.id) and text.lower().startswith("code:"):
...
PUBLIC_COMMANDS = [
    ...
    ("id","Mon ID"),("reset","Réinitialiser la mémoire"),("clear","Effacer la mémoire"),("mood","Humeur d'Alicia"),("ask","Poser une question"),
    ("games","Jeux"),("challenge","Défier un joueur"),("accept","Accepter un défi"),("score","Mon score"),("ranking","Classement"),
    ("missions","Missions Premium"),("premium","Premium à vie"),("coding","Codage 50 ⭐ / 24h"),("projet","Mes projets de codage"),
    ("referral","Mon parrainage"),("download","Télécharger une vidéo"),("video","Télécharger vidéo"),("song","Télécharger chanson"),("music","Rechercher une musique"),
    ...
    ("groupstats","Stats du groupe"),("top","Top du groupe"),("voice","Vocal Alicia"),("paysupport","Support paiement")
]

ADMIN_COMMANDS = PUBLIC_COMMANDS + [
    ("admin","Administration"),("stats","Statistiques"),("stars","Paiements Stars"),("users","Utilisateurs"),("groups","Groupes"),("user","Utilisateur"),
...
async def main():
    ...
    if url:
        ...
        await app.start()
        greeting_task = asyncio.create_task(daily_greetings_loop(app.bot))
