...
async def games(update, context):
    kb = [
        ...
        [InlineKeyboardButton("👤 Défier un joueur", callback_data="game:pvp")],
        [InlineKeyboardButton("🏆 Mon score", callback_data="game:score")],
    ]
...
async def challenge(update, context):
    if not context.args:
        await safe_reply(update.effective_message, "Utilise /challenge @username")
        return
    target = " ".join(context.args).strip()
    await safe_reply(update.effective_message, f"Défi envoyé à {target}. Le mode 1v1 sera lancé quand le joueur accepte avec /accept.")

async def accept(update, context):
    await safe_reply(update.effective_message, "Défi accepté. Choisissez un jeu avec /games.")

async def score_cmd(update, context):
    chat_id = update.effective_chat.id
    uid = update.effective_user.id
    points, wins, losses = get_score(chat_id, uid)
    await safe_reply(update.effective_message, f"🏆 Ton score\n\nPoints : {points}\nVictoires : {wins}\nDéfaites : {losses}")

async def ranking_cmd(update, context):
    con = db()
    rows = con.execute("SELECT name,points,wins,losses FROM scores WHERE chat_id=? ORDER BY points DESC LIMIT 10", (update.effective_chat.id,)).fetchall()
    con.close()
    if not rows:
        await safe_reply(update.effective_message, "Aucun score pour le moment.")
        return
    lines=["🏆 CLASSEMENT"]
    for i,(name,points,wins,losses) in enumerate(rows,1):
        lines.append(f"{i}. {name} — {points} pts | {wins}V / {losses}D")
    await safe_reply(update.effective_message, "\n".join(lines))
...
async def admin(update, context):
    ...
    await safe_reply(update.effective_message,
        ...
        "/groups — groupes connus\n"
        "/groupinfo — détail d'un groupe\n"
        "/broadcast texte — message privé\n"
        "/broadcastgroups texte — message aux groupes"
    )
...
async def game_callback(update, context):
    ...
    data = q.data or ""
    if data == "game:ai":
        await q.edit_message_text("🎮 Jeu contre Alicia\n\nPour l'instant, choisis un jeu avec /games. Les parties complètes seront ajoutées sans modifier tes statistiques.")
    elif data == "game:pvp":
        await q.edit_message_text("👤 Mode joueur contre joueur\n\nUtilise /challenge @username pour lancer un défi.")
    elif data == "game:score":
        points,wins,losses=get_score(update.effective_chat.id, update.effective_user.id)
