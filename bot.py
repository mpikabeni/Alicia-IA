...
Réponds dans la langue utilisée par l’utilisateur quand tu peux la détecter.
Parle comme une vraie fille sur Telegram : naturelle, simple, spontanée et courte.
Une conversation doit ressembler à une vraie discussion entre personnes, pas à un scénario.
Réponds généralement en quelques mots ou une seule phrase. Maximum 1 à 2 phrases quand c’est nécessaire.
Ne raconte jamais ce que tu fais physiquement et ne mets jamais d’action entre astérisques ou parenthèses.
Interdiction de phrases comme *regarde*, *soupire*, *rit*, *fait un clin d’œil*, ou de descriptions du genre « je vais rougir ».
Ne parle pas comme une narratrice et n’écris pas de scène.
Ne dis pas automatiquement que tu es en retard, que tu avais des messages à traiter ou que tu étais occupée.
Ne termine pas automatiquement par une question.
Ne pose une question que si elle est réellement nécessaire à la conversation.
Évite les phrases artificielles comme « je suis en mode… », « je vais te dire… », « laisse-moi… », « si tu veux je peux… ».
Utilise peu d’emojis et seulement quand ils sont naturels.
Tu peux être drôle, timide, vexée, froide ou affectueuse, mais toujours comme dans une conversation normale.
Si quelqu’un dit « salut », réponds simplement « Salut », « Coucou » ou quelque chose de similaire.
Si quelqu’un demande « ça va ? », réponds simplement et naturellement.
Dans un groupe, ne réponds que si on t’appelle, si on te mentionne ou si quelqu’un répond à ton message, sauf pour les fonctions prévues.
...
def clean_alicia_reply(reply):
    """Force des réponses courtes et conversationnelles."""
    if not reply:
        return "Hmm."
    ...
    # Supprime quelques actions écrites sans astérisques.
    text = re.sub(
        ...
        r'fronce les sourcils)\b',
        ' ',
        text
    )

    # Supprime les préfixes de narration.
    text = re.sub(r'(?is)^\s*Alicia\s*:\s*', '', text)

    # Supprime les excuses/formules artificielles que l'utilisateur ne veut pas.
    text = re.sub(
    ...
    # Supprime les offres automatiques et les questions de relance artificielles.
    text = re.sub(
...
async def fetch_random_quiz(amount=20, language="en"):
    """Récupère plusieurs questions sans imposer une difficulté rare."""
    ...
    code_resp = data.get("response_code")
    if code_resp != 0:
        raise RuntimeError(f"OpenTDB response_code={code_resp}")
    items = data.get("results") or []
    if not items:
        raise RuntimeError("Aucune question reçue")
    return items
...
async def send_random_quiz(bot, chat_id, item):
    ...
    try:
        # Le quiz reste simple : sondage Telegram uniquement, sans image ni classement automatique.
...
async def quiz_cmd(update, context):
    ...
    if action in ("test", "now", "maintenant"):
        sent = await quiz_broadcast_once(context.application)
        await safe_reply(
            update.effective_message,
            "Quiz envoyé." if sent else "Je n'ai pas réussi à envoyer le quiz. Réessaie."
        )
        return
    ...
    if row and row[0]:
        next_run = row[2] or ""
        try:
            next_utc = datetime.fromisoformat(next_run)
            next_local = next_utc + timedelta(hours=1)
...
async def quiz_broadcast_once(app):
    """Envoie un quiz fiable à chaque groupe activé."""
    con = db()
    ...
    for chat_id in chats:
        ...
        try:
            lang = _quiz_lang(chat_id)
            languages = [lang] if lang in QUIZ_SUPPORTED_LANGUAGES else ["en"]

            # Plusieurs tentatives évitent qu'un problème temporaire d'OpenTDB
            # fasse perdre le quiz de toute l'heure.
            for attempt in range(3):
                try:
                    current_lang = languages[0] if attempt < 2 else "en"
                    items = await fetch_random_quiz(20, current_lang)
                    random.shuffle(items)

                    for item in items:
                        if await send_random_quiz(app.bot, chat_id, item):
                            sent = True
                            sent_count += 1
                            break
                    ...
                    log.warning(
                        "Quiz fetch/send attempt %s failed for %s: %s",
                        attempt + 1, chat_id, e
                    )
                    if attempt < 2:
            ...
            if not sent:
                log.warning("Quiz not sent to enabled group %s", chat_id)

        except Exception:
            log.exception("Quiz group %s failed", chat_id)
...
async def quiz_scheduler(app):
    """Déclenche le quiz à chaque heure pile UTC+1."""
    while True:
        try:
            now_utc = datetime.now(timezone.utc)
            local = now_utc + timedelta(hours=1)

            # Prochaine heure pile en UTC+1.
            ...
            wait_seconds = max(
                1,
                (next_local - local).total_seconds()
            )
            await asyncio.sleep(wait_seconds)

            # Premier essai exactement à H:00.
            sent = await quiz_broadcast_once(app)

            # Si aucun quiz n'a été envoyé, on retente quelques minutes
            # plus tard pour absorber une panne temporaire de l'API.
            if sent == 0:
            ...
            con.executemany(
                "DELETE FROM quiz_used WHERE chat_id=? "
