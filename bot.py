...
from telegram import (
    Update, InlineKeyboardButton, InlineKeyboardMarkup, LabeledPrice, InputMediaPhoto,
...
RSS_AUTO_GROUPS = os.getenv("RSS_AUTO_GROUPS", "1").strip() not in {"0", "false", "no"}
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS rss_routes(
        ...
        active INTEGER DEFAULT 1,
        created_at TEXT,
        UNIQUE(feed_id, owner_user_id, destination_chat_id)
    );
...
def rss_list(user_id=None):
    con = db()
    if user_id is None:
        rows = con.execute("SELECT id,url,COALESCE(title,''),active,COALESCE(owner_user_id,0),COALESCE(destination_chat_id,'') FROM rss_feeds ORDER BY id").fetchall()
    else:
        rows = con.execute(
            "SELECT r.id,f.url,COALESCE(f.title,''),f.active,r.owner_user_id,r.destination_chat_id "
            "FROM rss_routes r JOIN rss_feeds f ON f.id=r.feed_id "
            "WHERE r.owner_user_id=? AND r.active=1 ORDER BY r.id",
            (user_id,),
        ).fetchall()
    con.close()
    return rows
...
async def rss_bot_is_admin(bot, chat_id):
    """Vérifie qu'Alicia est administratrice/creator du groupe ou canal."""
    try:
        me = await bot.get_me()
...
def rss_private_destinations():
    con = db()
    rows = con.execute("SELECT chat_id FROM rss_subscriptions WHERE enabled=1 AND chat_type='private'").fetchall()
    con.close()
    return [int(r[0]) for r in rows]

async def rss_group_destinations(bot):
    """Groupes/supergroupes où Alicia est admin uniquement."""
    if not RSS_AUTO_GROUPS:
        return []
    con = db()
    rows = con.execute("SELECT chat_id FROM chats WHERE chat_type IN ('group','supergroup')").fetchall()
    con.close()
    result = []
    for (chat_id,) in rows:
...
async def rss_channel_from_link(bot, raw):
    """Résout un canal public (@username ou https://t.me/username)."""
    target = (raw or "").strip()
    if target.startswith("https://t.me/") or target.startswith("http://t.me/"):
        tail = target.rstrip("/").split("/")[-1]
        if tail.startswith("+"):
            raise ValueError("Lien de canal privé/invitation : ajoute d'abord Alicia comme administratrice puis donne-moi le @username ou l'ID du canal.")
        target = "@" + tail.lstrip("@")
    if not target.startswith("@") and not re.fullmatch(r"-?\d+", target):
        target = "@" + target
    chat = await bot.get_chat(target)
    if getattr(chat, "type", "") != ChatType.CHANNEL:
        raise ValueError("Ce lien ne correspond pas à un canal Telegram.")
    if not await rss_bot_is_admin(bot, chat.id):
        raise ValueError("Alicia doit être administratrice du canal avec le droit de publier.")
    return chat

def rss_get_user_channel(user_id):
    con = db()
    row = con.execute("SELECT chat_id,title,username FROM rss_channels WHERE user_id=?", (user_id,)).fetchone()
    con.close()
    return row

def rss_set_user_channel(user_id, chat):
    con = db()
    con.execute(
        "INSERT INTO rss_channels(user_id,chat_id,title,username,updated_at) VALUES(?,?,?,?,?) "
        "ON CONFLICT(user_id) DO UPDATE SET chat_id=excluded.chat_id,title=excluded.title,username=excluded.username,updated_at=excluded.updated_at",
        (user_id, str(chat.id), getattr(chat, "title", "") or "", getattr(chat, "username", "") or "", now()),
    )
    con.commit(); con.close()

def rss_clear_user_channel(user_id):
    con = db(); con.execute("DELETE FROM rss_channels WHERE user_id=?", (user_id,)); con.commit(); con.close()

def rss_destination_label(bot, chat_id):
    return str(chat_id)

async def rss_cmd(update, context):
    chat = update.effective_chat
    user = update.effective_user
    args = [str(x).strip() for x in (context.args or [])]
    low = args[0].lower() if args else ""
    ...
    if chat and chat.type == ChatType.PRIVATE:
        ...
        if low == "off":
            ...
            return
        channel = rss_get_user_channel(user.id) if user else None
        feeds = rss_list(user.id) if user else []
        lines = [f"📰 RSS privé : {'activé' if rss_subscription(chat.id) else 'désactivé'}",
                 "Activer : /rss on", "Désactiver : /rss off",
                 "Configurer un canal : /rsschannel @MonCanal"]
        if channel:
            lines.append(f"Canal configuré : {channel[1] or channel[2] or channel[0]}")
        else:
            lines.append("Canal configuré : aucun")
        if feeds:
            lines.append("\nTes flux :")
            for fid, url, title, active, owner, destination in feeds:
                lines.append(f"{fid}. {title or url} → {destination or 'privé'}")
    ...
    if is_group(chat):
        if not await rss_bot_is_admin(context.bot, chat.id):
            await safe_reply(update.effective_message, "Le RSS ne peut être publié ici que si Alicia est administratrice.")
            return
        if low == "off":
            rss_unsubscribe(chat.id)
            await safe_reply(update.effective_message, "RSS désactivé pour ce groupe.")
            return
        if low == "on":
            rss_subscribe(chat.id, chat.type, user.id if user else None)
            await safe_reply(update.effective_message, "📰 RSS activé pour ce groupe. Alicia est administratrice, donc elle peut publier les nouveautés.")
            return
        await safe_reply(update.effective_message, "📰 RSS autorisé ici : Alicia est administratrice. /rss off pour désactiver les publications dans ce groupe.")
        return
    ...
    if not rows:
        ...
        return
    lines = ["📰 Flux RSS globaux :"]
    for fid, url, title, active, owner, destination in rows:
        lines.append(f"{fid}. {title or url} → {destination or 'auto'}")
    lines.append("\nAjouter : /addrss URL")
    lines.append("Supprimer : /delrss ID")
    lines.append("Tester : /rssnow")
    await safe_reply(update.effective_message, "\n".join(lines))

async def rsschannel_cmd(update, context):
    if not update.effective_user or not context.args:
        await safe_reply(update.effective_message, "Utilise : /rsschannel @MonCanal ou /rsschannel off")
        return
    if context.args[0].lower() == "off":
        rss_clear_user_channel(update.effective_user.id)
        await safe_reply(update.effective_message, "Canal RSS retiré de ta configuration.")
        return
    try:
        channel = await rss_channel_from_link(context.bot, context.args[0])
        rss_set_user_channel(update.effective_user.id, channel)
        label = getattr(channel, "username", "") or getattr(channel, "title", "") or str(channel.id)
        await safe_reply(update.effective_message, f"✅ Canal RSS configuré : {label}\nAlicia doit rester administratrice pour publier.")
    except Exception as e:
...
async def addrss_cmd(update, context):
    ...
    chat = update.effective_chat
    destination = ""
    owner = user.id if user else 0

    # En privé, le flux est lié au canal configuré par l'utilisateur.
    if chat and chat.type == ChatType.PRIVATE:
        channel = rss_get_user_channel(owner)
        if not channel:
            await safe_reply(update.effective_message, "Configure d'abord ton canal avec /rsschannel @MonCanal")
            return
        destination = channel[0]
    elif is_group(chat):
        if not await rss_bot_is_admin(context.bot, chat.id):
            await safe_reply(update.effective_message, "Alicia doit être administratrice de ce groupe pour y publier le flux.")
            return
        destination = str(chat.id)
    elif not rss_admin(update):
        return
    else:
        destination = rss_target_chat() or ""
    ...
    try:
        ...
        con = db()
        existing = con.execute("SELECT id FROM rss_feeds WHERE url=?", (url,)).fetchone()
        if existing:
            feed_id = int(existing[0])
        else:
            cur = con.execute(
                "INSERT INTO rss_feeds(url,title,active,created_at,owner_user_id,destination_chat_id) VALUES(?,?,?,?,?,?)",
                (url, title, 1, now(), owner, destination),
            )
            feed_id = int(cur.lastrowid)
        # Une route permet à plusieurs utilisateurs/canaux d'utiliser le même flux.
        con.execute(
            "INSERT OR IGNORE INTO rss_routes(feed_id,owner_user_id,destination_chat_id,active,created_at) VALUES(?,?,?,?,?)",
            (feed_id, owner, destination, 1, now()),
        )
        con.commit(); con.close()
...
async def delrss_cmd(update, context):
    ...
    if admin_ok(update):
        ...
        if not row:
            con.close(); await safe_reply(update.effective_message, "Flux introuvable."); return
        con.execute("DELETE FROM rss_routes WHERE feed_id=?", (fid,))
    ...
    row = con.execute("SELECT id,feed_id FROM rss_routes WHERE id=? AND owner_user_id=? AND active=1", (fid, user_id)).fetchone()
    if row:
        con.execute("UPDATE rss_routes SET active=0 WHERE id=?", (fid,))
        con.commit(); con.close()
        await safe_reply(update.effective_message, "Flux supprimé de ta configuration.")
        return

    # Compatibilité : si l'utilisateur a reçu l'ancien ID du flux, on peut supprimer sa route liée.
    row = con.execute("SELECT id FROM rss_routes WHERE feed_id=? AND owner_user_id=? AND active=1 LIMIT 1", (fid, user_id)).fetchone()
    if row:
        con.execute("UPDATE rss_routes SET active=0 WHERE id=?", (row[0],))
        con.commit(); con.close()
        await safe_reply(update.effective_message, "Flux supprimé de ta configuration.")
        return
    con.close()
    await safe_reply(update.effective_message, "Flux introuvable ou tu n'en es pas propriétaire.")
...
def rss_image_url(entry):
    ...
    for key in ("media_content", "media_thumbnail", "enclosures"):
        ...
        for item in values:
            if isinstance(item, dict):
                ...
                if url and (not mime or mime.startswith("image/")): candidates.append(url)
    for key in ("image", "thumbnail", "image_url"):
        value = getattr(entry, key, None)
        if isinstance(value, dict): value = value.get("href") or value.get("url")
    ...
    if match: candidates.append(html.unescape(match.group(1).strip()))
    for candidate in candidates:
        if re.match(r"^https?://[^\s<>\"']+$", candidate, re.I): return candidate
    return ""
...
async def rss_fetch_and_publish(bot, force=False):
    if feedparser is None: return 0
    base_destinations = set(await rss_group_destinations(bot))
    base_destinations.update(rss_private_destinations())
    rows = rss_list()
    sent_count = 0
    for fid, url, feed_title, active, owner, feed_destination in rows:
        if not active: continue
        destinations = set(base_destinations)
        if feed_destination:
            destinations.add(str(feed_destination))
        con_routes = db()
        route_rows = con_routes.execute("SELECT destination_chat_id FROM rss_routes WHERE feed_id=? AND active=1", (fid,)).fetchall()
        con_routes.close()
        destinations.update(str(r[0]) for r in route_rows if r[0])
        ...
        try:
            ...
            for entry in reversed(entries):
                ...
                con = db()
                con.execute("INSERT OR IGNORE INTO rss_items(url,feed_id,title,published_at,sent_at) VALUES(?,?,?,?,?)", (link, fid, title, str(getattr(entry, "published", "") or ""), now()))
                con.commit(); con.close()

                for target_chat in destinations:
                    key = str(target_chat)
                    con = db(); delivered = con.execute("SELECT 1 FROM rss_deliveries WHERE item_url=? AND chat_id=?", (link, key)).fetchone(); con.close()
                    ...
                    try:
                        ...
                        if ctype in (ChatType.GROUP, ChatType.SUPERGROUP, ChatType.CHANNEL):
                            if not await rss_bot_is_admin(bot, target_chat): continue
                        ...
                        if image_url:
                            ...
                            await safe_send_message(bot, target_chat, text, parse_mode="HTML", disable_web_page_preview=False)
                        con = db(); con.execute("INSERT OR IGNORE INTO rss_deliveries(item_url,chat_id,sent_at) VALUES(?,?,?)", (link,key,now())); con.commit(); con.close()
                        sent_count += 1
                    except Exception as e:
...
async def rssnow_cmd(update, context):
    if not rss_admin(update): return
...
async def rss_loop(bot):
    if feedparser is None:
        ...
        return
    while True:
        try: await rss_fetch_and_publish(bot)
        except Exception: log.exception("RSS loop error")
...
def game_menu():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("♟️ Échecs 1v1", callback_data="game:chess"),
         InlineKeyboardButton("🎲 Ludo 1v1", callback_data="game:ludo")],
...
async def accept(update, context):
    ...
    if time.time() - invite["created"] > 600:
        ...
        return
    GAME_INVITES.pop(key, None)
    players=[invite["from_id"], update.effective_user.id]
    names={players[0]:invite.get("from_name") or str(players[0]), players[1]:display_name(update.effective_user)}
    session={"opponent": invite["from_id"], "players": players, "names": names, "chat_id": invite["chat_id"]}
    GAME_SESSIONS[players[0]]=session
    GAME_SESSIONS[players[1]]=session
    await safe_reply(update.effective_message, "Défi accepté. Choisissez un jeu.", reply_markup=game_menu())

# ============================================================
# JEUX 1V1 AVANCÉS : ÉCHECS + LUDO GRAPHIQUE
# ============================================================
ADV_GAME_SEQ = 0
CHESS_SESSIONS = {}
LUDO_SESSIONS = {}
...
def _next_game_id():
    global ADV_GAME_SEQ
    ADV_GAME_SEQ = (ADV_GAME_SEQ + 1) % 100000
    return ADV_GAME_SEQ

def find_duel_for_user(uid, chat_id):
    for session in GAME_SESSIONS.values():
        if session.get("chat_id") == chat_id and uid in session.get("players", []):
            return session
    return None
...
def chess_board_image(board, selected=None):
    if Image is None or ImageDraw is None or chess is None:
        return None
    cell, margin = 82, 40
    img = Image.new("RGB", (760, 760), (16, 18, 24))
    d = ImageDraw.Draw(img)
    light, dark = (235, 225, 205), (83, 104, 91)
    piece_font, coord_font = game_font(58, True), game_font(20, True)
    symbols = {"P":"P","N":"N","B":"B","R":"R","Q":"Q","K":"K","p":"p","n":"n","b":"b","r":"r","q":"q","k":"k"}
    for row in range(8):
        for col in range(8):
            ...
            if piece:
                label = symbols.get(piece.symbol(), "?")
                color = (245, 245, 245) if piece.color else (25, 25, 30)
                stroke = (20, 20, 20) if piece.color else (220, 220, 220)
                d.text((x0 + cell // 2, y0 + cell // 2), label, font=piece_font, fill=color, anchor="mm", stroke_width=2, stroke_fill=stroke)
    for i, letter in enumerate("abcdefgh"):
        d.text((margin + i * cell + 6, margin + 8 * cell + 8), letter, font=coord_font, fill=(230, 230, 230))
    for i, number in enumerate("87654321"):
        d.text((8, margin + i * cell + 8), number, font=coord_font, fill=(230, 230, 230))
    return img

def chess_keyboard(session_id, board, selected=None):
    rows = []
    if selected is not None:
        legal = [m.to_square for m in board.legal_moves if m.from_square == selected]
        for i in range(0, len(legal), 4):
            rows.append([InlineKeyboardButton(chess.square_name(sq), callback_data=f"chess:{session_id}:{chess.square_name(selected)}:{chess.square_name(sq)}") for sq in legal[i:i + 4]])
        rows.append([InlineKeyboardButton("↩️ Annuler", callback_data=f"chess:{session_id}:cancel")])
    else:
        squares = []
        for move in board.legal_moves:
            if move.from_square not in squares:
                squares.append(move.from_square)
        for i in range(0, len(squares), 4):
            rows.append([InlineKeyboardButton(chess.square_name(sq), callback_data=f"chess:{session_id}:{chess.square_name(sq)}") for sq in squares[i:i + 4]])
    return InlineKeyboardMarkup(rows or [[InlineKeyboardButton("Fin", callback_data=f"chess:{session_id}:noop")]])

def chess_caption(state):
    names = state["names"]
    turn = state["players"][0] if state["board"].turn else state["players"][1]
    return f"♟️ ÉCHECS 1V1\n\n{names[state['players'][0]]} ⚪ — {names[state['players'][1]]} ⚫\n🎯 Tour : {names[turn]}"

async def send_chess_board(bot, chat_id, state, message=None):
    image = chess_board_image(state["board"], state.get("selected"))
    if image is None:
        return
    bio = io.BytesIO(); bio.name = "chess.png"; image.save(bio, "PNG"); bio.seek(0)
    markup = chess_keyboard(state["id"], state["board"], state.get("selected"))
    caption = chess_caption(state)
    if message is not None and getattr(message, "photo", None):
        await message.edit_media(InputMediaPhoto(bio, caption=caption), reply_markup=markup)
    else:
        await bot.send_photo(chat_id, bio, caption=caption, reply_markup=markup)

async def start_chess(update, context):
    uid, chat_id = update.effective_user.id, update.effective_chat.id
    duel = find_duel_for_user(uid, chat_id)
    if not duel or len(duel.get("players", [])) != 2:
        await q_or_reply(update, "⚠️ Lance d'abord un /challenge @joueur puis /accept.")
        return
    if chess is None:
        await q_or_reply(update, "⚠️ Le module d'échecs manque. Ajoute python-chess dans requirements.txt puis redéploie.")
        return
    players = duel["players"]; sid = _next_game_id()
    state = {"id": sid, "chat_id": chat_id, "players": players, "names": duel.get("names", {}), "board": chess.Board(), "selected": None}
    CHESS_SESSIONS[sid] = state
    await q_or_reply(update, f"♟️ Partie d'échecs créée. {state['names'][players[0]]} commence.")
    await send_chess_board(context.bot, chat_id, state)

def ludo_image(state):
    if Image is None or ImageDraw is None:
        return None
    W, ox, cell = 820, 70, 45
    img = Image.new("RGB", (W, W), (20, 22, 30)); d = ImageDraw.Draw(img)
    for r in range(15):
        for c in range(15):
            x, y = ox + c * cell, ox + r * cell
            fill = (70, 74, 84) if 5 <= r <= 9 and 5 <= c <= 9 else (38, 42, 52)
            d.rectangle((x, y, x + cell - 2, y + cell - 2), fill=fill)
    colors = [(220, 70, 70), (70, 130, 230)]
    for idx, color in enumerate(colors):
        base = (ox + 65, ox + 65) if idx == 0 else (ox + 14 * cell - 65, ox + 14 * cell - 65)
        for token in range(4):
            tx = base[0] + (token % 2) * 30; ty = base[1] + (token // 2) * 30
            d.ellipse((tx - 14, ty - 14, tx + 14, ty + 14), fill=color, outline=(255, 255, 255), width=2)
        for step in range(20):
            x = ox + ((1 + step) % 15) * cell; y = ox + (2 + idx * 10) * cell
            d.ellipse((x + 8, y + 8, x + cell - 9, y + cell - 9), outline=color, width=3)
    d.text((30, 20), "LUDO 1V1", font=game_font(28, True), fill=(245, 245, 245))
    d.text((30, 780), f"Tour : {state['names'][state['players'][state['turn']]]}", font=game_font(22, True), fill=(245, 245, 245))
    return img

def ludo_keyboard(sid, state):
    if state.get("rolled") is None:
        return InlineKeyboardMarkup([[InlineKeyboardButton("🎲 Lancer le dé", callback_data=f"ludo:{sid}:roll")]])
    return InlineKeyboardMarkup([[InlineKeyboardButton(f"Pion {i + 1}", callback_data=f"ludo:{sid}:move:{i}") for i in range(4)]])

async def send_ludo_board(bot, chat_id, state, message=None):
    image = ludo_image(state)
    if image is None:
        return
    bio = io.BytesIO(); bio.name = "ludo.png"; image.save(bio, "PNG"); bio.seek(0)
    cap = f"🎲 LUDO 1V1\n{state['names'][state['players'][0]]} 🔴 — {state['names'][state['players'][1]]} 🔵\nDé : {state.get('rolled', '—')}"
    if message is not None and getattr(message, "photo", None):
        await message.edit_media(InputMediaPhoto(bio, caption=cap), reply_markup=ludo_keyboard(state['id'], state))
    else:
        await bot.send_photo(chat_id, bio, caption=cap, reply_markup=ludo_keyboard(state['id'], state))

async def start_ludo(update, context):
    uid, chat_id = update.effective_user.id, update.effective_chat.id
    duel = find_duel_for_user(uid, chat_id)
    if not duel:
        await q_or_reply(update, "⚠️ Lance d'abord un duel avec /challenge @joueur puis /accept.")
        return
    sid = _next_game_id(); players = duel["players"]
    state = {"id": sid, "chat_id": chat_id, "players": players, "names": duel.get("names", {}), "turn": 0, "rolled": None, "steps": [[0, 0, 0, 0], [0, 0, 0, 0]]}
    LUDO_SESSIONS[sid] = state
    await q_or_reply(update, "🎲 Ludo lancé. Le premier joueur commence.")
    await send_ludo_board(context.bot, chat_id, state)

async def q_or_reply(update, text):
    if getattr(update, "callback_query", None):
        await update.callback_query.message.reply_text(text)
    else:
        await safe_reply(update.effective_message, text)

async def chess_callback(update, context):
    q = update.callback_query; parts = (q.data or "").split(":"); await q.answer()
    if len(parts) < 3:
        return
    try: sid = int(parts[1])
    except ValueError: return
    state = CHESS_SESSIONS.get(sid)
    if not state or q.from_user.id not in state["players"]:
        return
    board = state["board"]; uid = q.from_user.id
    expected = state["players"][0] if board.turn else state["players"][1]
    if uid != expected:
        await q.answer("Ce n'est pas ton tour.", show_alert=True); return
    if parts[2] == "cancel":
        state["selected"] = None; await send_chess_board(context.bot, q.message.chat_id, state, q.message); return
    if parts[2] == "noop": return
    try:
        if len(parts) == 3:
            sq = chess.parse_square(parts[2])
            if board.piece_at(sq) and board.piece_at(sq).color == board.turn:
                state["selected"] = sq; await send_chess_board(context.bot, q.message.chat_id, state, q.message)
            return
        move = chess.Move.from_uci(parts[2] + parts[3])
        if move not in board.legal_moves:
            await q.answer("Coup impossible.", show_alert=True); return
        board.push(move); state["selected"] = None
        if board.is_checkmate():
            winner = uid; loser = state["players"][1 - (0 if uid == state["players"][0] else 1)]
            add_score(q.message.chat_id, winner, display_name(q.from_user), 30, win=True); add_score(q.message.chat_id, loser, "", 0, loss=True)
            await add_xp(context.bot, winner, 40, display_name(q.from_user)); await q.message.reply_text(f"♟️ Échec et mat. {display_name(q.from_user)} gagne !")
            CHESS_SESSIONS.pop(sid, None)
        elif board.is_stalemate() or board.is_insufficient_material():
            await q.message.reply_text("♟️ Partie nulle."); CHESS_SESSIONS.pop(sid, None)
        await send_chess_board(context.bot, q.message.chat_id, state, q.message)
    except Exception:
        await q.answer("Coup invalide.", show_alert=True)
...
async def ludo_callback(update, context):
    ...
    state = LUDO_SESSIONS.get(sid)
    if not state or q.from_user.id != state["players"][state["turn"]]:
        await q.answer("Ce n'est pas ton tour.", show_alert=True); return
    action = parts[2]
    if action == "roll":
        state["rolled"] = random.randint(1, 6); await send_ludo_board(context.bot, q.message.chat_id, state, q.message); return
    if action == "move" and len(parts) >= 4 and state.get("rolled") is not None:
        try: token = int(parts[3])
        except ValueError: return
        if token not in range(4): return
        idx, roll = state["turn"], state["rolled"]
        state["steps"][idx][token] = min(20, state["steps"][idx][token] + roll); state["rolled"] = None
        if all(x >= 20 for x in state["steps"][idx]):
            uid = q.from_user.id; add_score(q.message.chat_id, uid, display_name(q.from_user), 30, win=True); await add_xp(context.bot, uid, 40, display_name(q.from_user)); await q.message.reply_text(f"🎲 {display_name(q.from_user)} remporte le Ludo !"); LUDO_SESSIONS.pop(sid, None); return
        state["turn"] = 1 - idx; await send_ludo_board(context.bot, q.message.chat_id, state, q.message)
...
async def game_callback(update, context):
    ...
    if game == "chess":
        ...
        await start_ludo(update, context)
    elif game == "ttt":
...
PUBLIC_COMMANDS = [
    ...
    ("motivate","Motivation"), ("rss","Flux RSS"), ("rsschannel","Canal RSS"), ("addrss","Ajouter un flux RSS"), ("delrss","Supprimer un flux RSS"), ("groupinfo","Infos du groupe"), ("groupstats","Stats du groupe"),
...
ADMIN_COMMANDS = PUBLIC_COMMANDS + [
    ...
    ("rssnow","Publier les nouveautés RSS"),
]
...
def build_app():
    ...
    app.add_handler(CommandHandler("rsschannel", rsschannel_cmd))
