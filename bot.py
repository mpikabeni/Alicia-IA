...
async def rss_fetch_and_publish(bot, force=False):
    if feedparser is None or not RSS_CHANNEL:
        return 0
    rows = rss_list()
    sent_count = 0
        ...
        try:
            ...
            for entry in reversed(entries):
                ...
                if not link or not title:
                    continue
                con = db()
                exists = con.execute("SELECT 1 FROM rss_items WHERE url=?", (link,)).fetchone()
                if exists:
                    con.close(); continue
                con.execute("INSERT INTO rss_items(url,feed_id,title,published_at,sent_at) VALUES(?,?,?,?,?)", (link, fid, title, str(getattr(entry, "published", "") or ""), now()))
                con.commit(); con.close()
                ...
                try:
                    await safe_send_message(bot, RSS_CHANNEL, text, parse_mode="HTML", disable_web_page_preview=False)
                    sent_count += 1
                except Exception as e:
                    log.warning("RSS publish failed: %s", e)
        except Exception as e:
...
PUBLIC_COMMANDS = [
    ...
    ("score","Mon score"), ("ranking","Classement"), ("missions","Missions Premium"), ("premium","Premium à vie"),
    ...
    ("motivate","Motivation"), ("groupinfo","Infos du groupe"), ("groupstats","Stats du groupe"),
...
ADMIN_COMMANDS = PUBLIC_COMMANDS + [
    ...
    ("rss","Flux RSS"), ("addrss","Ajouter un flux RSS"), ("delrss","Supprimer un flux RSS"), ("rssnow","Publier les nouveautés RSS")
]
